...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        <title>投稿したギルクエ / MH4G情報管理.com</title>
    ...
    <body class="bg-gray-200">
        <div class="w-[92%] min-h-[85vh] mx-auto bg-white shadow">
            <div class="w-[95%] mx-auto  space-y-6">
                <h1 class="text-4xl font-semibold py-6" id='title'>マイギルクエ管理</h1>
                ...
                <form id="sort_filter" action="/posts/myposts" method="GET" class="inline-block">
                    @csrf
                ...
                <table  class="w-full text-center border-collapse border divide-x divide-y shadow">
                    ...
                    @foreach ($posts as $post)
                        <tr class="border divide-x divide-y odd:bg-gray-100 even:bg-white grandparent-container">
                            ...
                            <td class="detail py-2.5 ">
                                <button type="button" class="bg-gray-100 hover:bg-gray-300 text-blue-500 font-bold py-2 px-2  border-2 border-gray-400 rounded " onclick="location.href='/posts/{{ $post->id }}/edit'">
                                    編集
                                </button>
                            </td>
                            <td class="detail py-2.5 ">
                                <form action="/posts/{{ $post->id }}" id="form_{{ $post->id }}" method="post">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="deletePost({{ $post->id }})" class="bg-gray-100 hover:bg-gray-300 text-red-600 font-bold py-2 px-2 border-2 border-gray-400 rounded ">削除</button> 
                                </form>
                            </td>
                        </tr>
